...
/**
 * initPrism(container, options) — vanilla JS port of the Prism React component.
...
export function initPrism(container, options = {}) {
  ...
  // Cap DPR to 1 — the background shader doesn't need retina resolution
  const dpr = 1;
  ...
  const stopRAF  = () => { if (raf) { cancelAnimationFrame(raf); clearTimeout(raf); raf = 0; } };
  ...
  const render = (t) => {
    ...
    if (animationType === 'hover') {
      ...
      const tY = (pointer.inside ? -pointer.x : 0) * maxY;
      const tP = (pointer.inside ?  pointer.y : 0) * maxP;
      yaw   = lerp(yaw,   tY, INERT);
      pitch = lerp(pitch, tP, INERT);
      roll  = lerp(roll,  0,  0.1);
      program.uniforms.uRot.value = setMat3(yaw, pitch, roll, rotBuf);
      if (NOISE_IS_ZERO) {
        const settled = Math.abs(yaw-tY) < 1e-4 && Math.abs(pitch-tP) < 1e-4 && Math.abs(roll) < 1e-4;
      ...
      // 'rotate' mode — use built-in wob (uUseBaseWobble = 1), identity matrix
    ...
    // Throttle to ~30fps — background doesn't need 60fps, frees main thread
    raf = continueRAF ? setTimeout(() => { raf = requestAnimationFrame(render); }, 33) : 0;
  };
